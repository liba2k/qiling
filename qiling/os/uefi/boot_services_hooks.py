...
import qiling.os.uefi.uefi_types_64 as uefi_types
...
def SignalEvent(ql, event_id):
    if event_id in ql.loader.events:
        ...
        if not event["Set"]:
            ...
            if ql.loader.notify_immediately:
                ...
                ql.loader.notify_return_address = ql.stack_pop()
...
def hook_CheckEvent(ql, address, params):
    return EFI_SUCCESS if ql.events[params["Event"]]["Set"] else EFI_NOT_READY
...
def hook_InstallConfigurationTable(ql, address, params):
    guid = params["Guid"]
    uuid = UUID(guid)
    efi_configuration_table = uefi_types.EFI_CONFIGURATION_TABLE()
...
def hook_UninstallMultipleProtocolInterfaces(ql, address, params):
    ...
    if handle not in ql.loader.handle_dict:
        return EFI_NOT_FOUND
    index = 1
    while get_param_by_index(ctx, index) != 0:
        GUID_ptr = get_param_by_index(ctx, index)
        protocol_ptr = get_param_by_index(ctx, index+1)
        GUID = str(read_guid(ql, GUID_ptr))
        ql.nprint(f'\t {GUID}, {protocol_ptr:x}')
        dic = ql.loader.handle_dict[handle]
        protocol = params["Protocol"]
...
def hook_CalculateCrc32(ql, address, params):
    ...
    write_int32(params['Crc32'], crc32(data))
    return EFI_SUCCESS
...
def hook_EFI_BOOT_SERVICES(ql, start_ptr):
    ...
    efi_boot_services = uefi_types.EFI_BOOT_SERVICES()
    ptr = start_ptr
